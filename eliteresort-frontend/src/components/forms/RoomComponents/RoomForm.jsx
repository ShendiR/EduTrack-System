...
const RoomForm = ({ newRoom, setNewRoom, onAdd }) => (
  <div className="animate-in fade-in duration-700">
    <div className="flex items-center gap-3 mb-8">
      <div className="bg-slate-900 p-2 rounded-lg text-white"><Hotel size={20} /></div>
      <h2 className="text-xs font-bold uppercase tracking-[0.3em] text-slate-400">New Accommodation</h2>
    </div>
    ...
    <form onSubmit={onAdd} className="space-y-6">
      ...
      <div className="relative">
        <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Room Category (ID)</label>
        <select 
          ...
          required
        >
          <option value="">Select ID from Database</option>
          <option value="1">1 - Standard</option>
          <option value="2">2 - Deluxe</option>
          <option value="3">3 - Suite</option>
        </select>
        <ChevronDown className="absolute right-4 top-10 text-slate-300" size={16} />
      </div>
